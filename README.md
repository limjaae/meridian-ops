...
2. **Asia-Pacific Semiconductor Crisis (demo scenario)** — a synthetic but realistically-scaled
   ...
   suppliers) used to demonstrate the full Mission Control → Incident Workspace → Decision
...
- **Incidents** (`/incidents`, `/incidents/[id]`) — root cause, quantified impact, response
  ...
  workspace."
- **Decisions** (`/decisions`) — the decision register: every choice, its owner, and its reasoning,
  kept after the meeting where it was made is over.
